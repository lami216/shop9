...
const CheckoutPage = () => {
        ...
        const handleSubmit = async (event) => {
                ...
                const STORE_WHATSAPP_NUMBER = "22244138113";
        ...
        return (
                <div className='min-h-screen bg-ali-bg py-10 text-ali-ink'>
                        <div className='mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 lg:flex-row'>
                                <motion.section
                                        className='w-full rounded-xl bg-white p-6 shadow-sm ring-1 ring-ali-card'
                                ...
                                >
                                        ...
                                        <form className='space-y-5' onSubmit={handleSubmit}>
                                                ...
                                                <div className='space-y-2'>
                                                        <label className='block text-sm font-medium text-ali-ink' htmlFor='whatsAppNumber'>
                                                ...
                                                <motion.button
                                                        ...
                                                        className='w-full rounded-lg bg-gradient-to-r from-ali-red to-ali-rose px-5 py-3 text-base font-semibold text-white shadow transition duration-300 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-ali-rose/40 disabled:cursor-not-allowed disabled:opacity-50'
                                ...
                                <motion.aside
                                        className='w-full rounded-xl bg-white p-6 text-ali-ink shadow-sm ring-1 ring-ali-card lg:max-w-sm'
                                        initial={{ opacity: 0, x: 20 }}
                                        animate={{ opacity: 1, x: 0 }}
                                        transition={{ duration: 0.4, delay: 0.1 }}
                                >
                                        <h2 className='text-xl font-semibold text-ali-ink'>{t("checkout.summary.title")}</h2>
                                        <ul className='mt-4 space-y-3 text-sm text-ali-muted'>
                                                {cart.map((item) => {
                                                        const { price, discountedPrice, isDiscounted } = getProductPricing(item);
                                                        return (
                                                                <li key={item._id} className='flex justify-between gap-4'>
                                                                        <span className='font-medium text-ali-ink'>{item.name}</span>
                                        ...
                                        <div className='mt-6 space-y-2 border-t border-ali-card pt-4 text-sm text-ali-muted'>
                                                <div className='flex justify-between'>
                                                        <span>{t("checkout.summary.subtotal")}</span>
                                                        <span>{formatMRU(subtotal)}</span>
                                                </div>
                                                {savings > 0 && (
                                                        <div className='flex justify-between text-ali-red'>
                                                                <span>{t("checkout.summary.savings")}</span>
                                                                <span>-{formatMRU(savings)}</span>
                                                        </div>
                                                )}
                                                <div className='flex justify-between text-base font-semibold text-ali-ink'>
                                                        <span>{t("checkout.summary.total")}</span>
                                                        <span>{formatMRU(total)}</span>
                                                </div>
                                        </div>

                                        <p className='mt-4 text-xs text-ali-muted'>{t("checkout.summary.notice")}</p>
                                </motion.aside>
                        </div>
                </div>
        );
